...
FusionEKF::FusionEKF() {
  ...
  float dt = 1.0; //0.05; //  LS TODO check this
  ekf_.F_ << 1, 0, dt, 0,
             0, 1, 0, dt,
             0, 0, 1, 0,
             0, 0, 0, 1;

  // 4x4 Initial covariance matrix
...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    */
    ...
    //ekf_.x_ << 1, 1, 0, 0; // TODO LS try this
  ...
  if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
    ...
    ekf_.R_ = R_radar_; // just using R_radar

    ekf_.UpdateEKF(measurement_pack.raw_measurements_);
  } else {
    // Laser updates

    ekf_.H_ = H_laser_; // just using H_laser_
    ekf_.R_ = R_laser_; // just using R_lidar
